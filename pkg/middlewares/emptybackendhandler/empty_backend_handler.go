...
type emptyBackend struct {
	healthcheck.StatusBalancerHandler
}

// New creates a new EmptyBackend middleware.
func New(lb healthcheck.StatusBalancerHandler) http.Handler {
	return &emptyBackend{StatusBalancerHandler: lb}
}
...
func (e *emptyBackend) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
	if len(e.StatusBalancerHandler.Servers()) != 0 {
		e.StatusBalancerHandler.ServeHTTP(rw, req)
		return
	}
